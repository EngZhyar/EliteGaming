...
sendGamesBtn.addEventListener("click", function() {
    ...
    if (!messageBox) {
        ...
        // Create WhatsApp send button (First number - 9647501238780)
        const whatsappBtn = document.createElement('button');
        whatsappBtn.textContent = 'داواکردنی یاری بۆ پلەی دوو';
        ...
        // Create second WhatsApp send button (New number - 07501234567)
        const whatsappBtn2 = document.createElement('button');
        whatsappBtn2.textContent = 'داواکردنی یاری بۆ کۆمپیوتەر';
        whatsappBtn2.style.cssText = `
            ...
            transition: 0.3s;
        `;
        whatsappBtn2.onmouseover = function() {
            this.style.transform = 'scale(1.02)';
        };
        whatsappBtn2.onmouseout = function() {
            this.style.transform = 'scale(1)';
        };
        whatsappBtn2.onclick = function() {
            const text = document.getElementById('messageText').value;
            const encodedMessage = encodeURIComponent(text);
            const phoneNumber = '7518979796';
            const whatsappURL = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
            window.open(whatsappURL, '_blank');
        };
        messageBox.appendChild(whatsappBtn2);
